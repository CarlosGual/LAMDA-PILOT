...
class Learner(BaseLearner):
    ...
    def after_task(self):
        for cls in self._selected_train_classes:
    ...
    def incremental_train(self, data_manager):
        ...
        self._selected_train_classes = selected_train_classes

        for i in self._selected_train_classes:
            self.cls2task[i] = self._cur_task

        self.data_manager = data_manager

        self._nb_classes = data_manager.nb_classes
        self._network.update_fc(len(selected_train_classes))
        ...
        self.replace_fc(self.train_loader_for_protonet, self._network, None)
        if len(self._multiple_gpus) > 1:
    ...
    def _init_train(self, train_loader, test_loader, optimizer, scheduler):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                ...
                logits = output["logits"][:, :len(self._selected_train_classes)]
                logits[:, :len(self._known_classes)] = float('-inf')
    ...
    def _compute_mean(self, model):
        model.eval()
        for class_idx in self._selected_train_classes:
    ...
    def classifer_align(self, model):
        ...
        for epoch in prog_bar:
            ...
            if self.args["ca_storage_efficient_method"] in ['covariance', 'variance']:
                for class_idx in self._known_classes:
            ...
            losses = 0.0
            correct, total = 0, 0
            for _iter in self._known_classes:
                inp = inputs[_iter * num_sampled_pcls:(_iter + 1) * num_sampled_pcls]
                tgt = targets[_iter * num_sampled_pcls:(_iter + 1) * num_sampled_pcls]
                outputs = model(inp, fc_only=True)
                logits = outputs['logits'][:, :len(self._known_classes)]
            ...
            info = "Task {}, Epoch {}/{} => Loss {:.3f}, CA_accy {:.2f}".format(
                ...
                epoch + 1,
                self.crct_epochs,
                losses / len(self._known_classes),
                ca_acc,
            )
            prog_bar.set_description(info)
    ...
    def _eval_cnn(self, loader):
        ...
        for _, (_, inputs, targets) in enumerate(loader):
            ...
            with torch.no_grad():
                orig_logits = self._network.forward_orig(inputs)["logits"][:, :len(self._known_classes)]
                ...
                for x_id in range(len(inputs)):
                    ...
                    while True:
                        ...
                        cur_logits = self._network.backbone(cur_feature, fc_only=True)["logits"][:, :len(self._known_classes)]
